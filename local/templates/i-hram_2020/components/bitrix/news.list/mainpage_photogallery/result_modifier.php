...
$arSect = CIBlockSection::GetList(
    ...
    false,
    array("ID", "NAME", "CODE", "PICTURE", "SECTION_PAGE_URL", "UF_DATE", "DESCRIPTION"),
    false);
while ($arSect_res = $arSect->GetNext()) {
    $arResult["SECTION"][$cnt]["ID"] = $arSect_res["ID"];
    $arResult["SECTION"][$cnt]["NAME"] = $arSect_res['NAME'];
    $arResult["SECTION"][$cnt]["DESCRIPTION"] = $arSect_res['~DESCRIPTION'];
    $arResult["SECTION"][$cnt]["DATE"] = $arSect_res['UF_DATE'];
    $arResult["SECTION"][$cnt]["URL"] = $arSect_res['SECTION_PAGE_URL'] . $arSect_res['CODE'] . "/";

    $img = CFile::GetFileArray($arSect_res['PICTURE']);
    $arResult["SECTION"][$cnt]["PICTURE"]["SRC"] = $img["SRC"];
    $arResult["SECTION"][$cnt]["PICTURE"]["DESCRIPTION"] = $img["DESCRIPTION"];

    $cnt++;
    if ($cnt >= $arParams["NEWS_COUNT"])
        break;
}